...
import { APMTracingSettings, DEFAULT_APM_SETTINGS, TraceData } from './types';
import { 
  generateTraceUrl, 
...
interface APMTracingComponentProps {
  context: PluginContext;
}

export const APMTracingComponent: React.FC<APMTracingComponentProps> = ({ context }) => {
  ...
  const pluginSettings = (currentPlugin?.settings as APMTracingSettings) || DEFAULT_APM_SETTINGS;

  // Form for APM configuration
  const form = useForm<APMTracingSettings>({
  ...
  const handleSaveSettings = async (values: APMTracingSettings) => {
    try {
      setIsLoading(true);
            ...
            <Stack gap="md">
              ...
              <Group justify="space-between">
                <Select
                  ...
                  onChange={(value) => form.setFieldValue('filterByStatus', value as any)}
                  data={[
