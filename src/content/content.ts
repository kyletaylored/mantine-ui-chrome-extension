...
async function handleContentMessage(request: any, sender: chrome.runtime.MessageSender, sendResponse: (response: any) => void) {
  try {
    switch (request.type) {
...
function injectDatadogScript(script: string): void {
...
function getPageInfo() {
  return {
    ...
    timestamp: Date.now(),
    hasDatadog: {
      rum: typeof (window as any).DD_RUM !== 'undefined',
      logs: typeof (window as any).DD_LOGS !== 'undefined'
    }
  };
}
...
function highlightElements(selector: string): void {
  ...
  elements.forEach(el => {
    ...
    (el as HTMLElement).style.outline = '2px solid #632CA6';
  });
...
function showInPageNotification(config: any): void {
