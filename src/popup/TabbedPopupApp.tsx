...
export function TabbedPopupApp() {
  ...
  const [activeTab, setActiveTab] = useState<string | null>('rum');
  const [rumSessionData, setRumSessionData] = useState<RumSessionData>({ isActive: false });
  const [apmTraces, setApmTraces] = useState<TraceData[]>([]);
  ...
  const openTraceInDatadog = (traceId: string) => {
            ...
            <Stack gap="md">
              ...
              <Stack gap="sm">
                ...
                {filteredLinks.length === 0 ? (
                  ...
                  <Stack gap="xs">
                    {filteredLinks.slice(0, 8).map((link: HelpfulLink) => (
